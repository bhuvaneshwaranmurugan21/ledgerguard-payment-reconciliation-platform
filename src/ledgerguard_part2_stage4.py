"""Read-only local entry point for Part 2 Stage 4 transaction reconciliation."""
...
def main() -> None:
    ...
    try:
        ...
        reconciled = reconcile_transactions(admitted)
        ...
        raise SystemExit(2) from error
    result = {
        "outcome": "TRANSACTION_RECONCILIATION_CANDIDATE",
        ...
        "transaction_candidate_count": len(reconciled.candidates),
        "transaction_candidates": [candidate.value() for candidate in reconciled.candidates],
        "transaction_state_sha256": reconciled.state.semantic_digest(),
