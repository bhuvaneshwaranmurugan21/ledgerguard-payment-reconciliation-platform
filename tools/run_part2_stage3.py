#!/usr/bin/env python3
"""Run Part 2 Stage 3 in exact, independent, reproducible environments."""
...
def run_clean(
    ...
    for lock in ("part2-stage3-bootstrap.lock", "part2-stage3-py311.lock"):
        execute(
            [
                str(python),
                "-m",
    ...
    if len(wheels) != 1:
        raise SystemExit("Stage 3 expected exactly one LedgerGuard wheel")
    ...
    execute(
        [
            str(python),
            str(source / "tools/validate_part2_stage3_run.py"),
            "--root",
            str(source),
            "--output",
            str(result_path),
...
def main() -> None:
    ...
    if arguments.clean_runs < 2:
        raise SystemExit("P2-S3-R021 requires at least two clean runs")
    if sys.version_info[:3] != (3, 11, 13):
        raise SystemExit("Part 2 Stage 3 requires exact CPython 3.11.13")
    repository = Path(__file__).resolve().parents[1]
    owned_temporary: tempfile.TemporaryDirectory[str] | None = None
    if arguments.output is None:
        owned_temporary = tempfile.TemporaryDirectory(prefix="ledgerguard-part2-stage3-")
        ...
        if repository == workspace or repository in workspace.parents:
            raise SystemExit("Part 2 Stage 3 output must be outside the repository")
        workspace.mkdir(parents=True, exist_ok=True)
    wheelhouses = [path.resolve() for path in arguments.wheelhouse]
    if any(not path.is_dir() for path in wheelhouses):
        raise SystemExit("Stage 3 wheelhouse does not exist")
    results = [
    ...
    if any(canonical(row) != first for row in deterministic[1:]):
        raise SystemExit("P2-S3-R021 clean-run deterministic payloads differ")
    evidence = {
        "schema_version": "1.0",
    ...
    destination = workspace / "part2-stage3-local-evidence.json"
