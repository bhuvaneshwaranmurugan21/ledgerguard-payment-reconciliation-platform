#!/usr/bin/env python3
"""Run Part 2 Stage 6 in exact, independent, reproducible environments."""
...
def run_clean(
    ...
    for lock in ("part2-stage6-bootstrap.lock", "part2-stage6-py311.lock"):
        execute(
            [
                str(python),
                "-m",
    ...
    execute(
        [
            str(python),
            str(source / "tools/validate_part2_stage6_run.py"),
            "--root",
            str(source),
            "--output",
            str(result_path),
...
def main() -> None:
    ...
    if arguments.clean_runs < 2:
        raise SystemExit("P2-S6-R029 requires at least two clean runs")
    if sys.version_info[:3] != (3, 11, 13):
        raise SystemExit("Part 2 Stage 6 requires exact CPython 3.11.13")
    repository = Path(__file__).resolve().parents[1]
    owned_temporary: tempfile.TemporaryDirectory[str] | None = None
    if arguments.output is None:
        owned_temporary = tempfile.TemporaryDirectory(prefix="ledgerguard-part2-stage6-")
        ...
        if repository == workspace or repository in workspace.parents:
            raise SystemExit("Part 2 Stage 6 output must be outside the repository")
    ...
    if any(canonical(row) != first for row in deterministic[1:]):
        raise SystemExit("P2-S6-R029 clean-run deterministic payloads differ")
    evidence = {
        "schema_version": "1.0",
    ...
    destination = workspace / "part2-stage6-local-evidence.json"
