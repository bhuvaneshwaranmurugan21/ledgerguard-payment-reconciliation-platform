#!/usr/bin/env python3
"""Build immutable exact-head CI evidence for a draft Stage 4 pull request."""
...
def main() -> None:
    ...
    if not isinstance(pull_request, dict):
        raise SystemExit("Stage 4 CI evidence requires a pull_request event")
    if pull_request.get("draft") is not True:
        raise SystemExit("Stage 4 evidence must be built while the pull request is draft")
    if pull_request["base"]["sha"] != "47e96d3f4846d55568c0b137fb3e4d41ead0eef1":
        raise SystemExit("Stage 4 pull request base is not the frozen Stage 3 closure")
    ...
    envelope: dict[str, Any] = {
        ...
        "stage4_candidate_digest": authority["stage4_candidate_digest"],
        ...
        "stage3_closure_commit": authority["stage3_closure"]["commit"],
        "stage3_external_state": authority["stage3_closure"]["state"],
    ...
    schema = json.loads((ROOT / "spec/part2-stage4-ci-evidence-v1.schema.json").read_text())
    Draft202012Validator.check_schema(schema)
    errors = sorted(
        Draft202012Validator(schema).iter_errors(envelope), key=lambda row: list(row.path)
    )
    if errors:
        raise SystemExit(f"Stage 4 CI evidence is invalid: {errors[0].message}")
    ...
    evidence_path = destination / "part2-stage4-ci-evidence.json"
