#!/usr/bin/env python3
"""Build immutable exact-head CI evidence for a draft Stage 5 pull request."""
...
def main() -> None:
    ...
    if not isinstance(pull_request, dict):
        raise SystemExit("Stage 5 CI evidence requires a pull_request event")
    if pull_request.get("draft") is not True:
        raise SystemExit("Stage 5 evidence must be built while the pull request is draft")
    if pull_request["base"]["sha"] != "c423ae7e6e92d37ffa8a796b4efacbf9ba6692f1":
        raise SystemExit("Stage 5 pull request base is not the frozen Stage 4 closure")
    ...
    envelope: dict[str, Any] = {
        ...
        "stage5_candidate_digest": authority["stage5_candidate_digest"],
        ...
        "stage4_closure_commit": authority["stage4_closure"]["commit"],
        "stage4_external_state": authority["stage4_closure"]["state"],
        "financial_invariants_state": authority["master_part2_gates"][
    ...
    schema = json.loads((ROOT / "spec/part2-stage5-ci-evidence-v1.schema.json").read_text())
    Draft202012Validator.check_schema(schema)
    errors = sorted(
        Draft202012Validator(schema).iter_errors(envelope), key=lambda row: list(row.path)
    )
    if errors:
        raise SystemExit(f"Stage 5 CI evidence is invalid: {errors[0].message}")
    ...
    evidence_path = destination / "part2-stage5-ci-evidence.json"
