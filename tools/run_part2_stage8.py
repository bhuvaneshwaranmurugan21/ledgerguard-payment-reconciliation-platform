#!/usr/bin/env python3
"""Run Part 2 Stage 8 twice in exact independent environments."""
...
def run_clean(
    ...
    if len(wheels) != 1:
        raise SystemExit("Stage 8 expected exactly one LedgerGuard wheel")
    ...
    execute(
        [
            str(python),
            str(source / "tools/validate_part2_stage8_run.py"),
            "--root",
            str(source),
            "--output",
            str(result_path),
...
def main() -> None:
    ...
    if args.clean_runs < 2:
        raise SystemExit("P2-S8-R016 requires at least two clean runs")
    if sys.version_info[:3] != (3, 11, 13):
        raise SystemExit("Part 2 Stage 8 requires exact CPython 3.11.13")
    repository = Path(__file__).resolve().parents[1]
    owned: tempfile.TemporaryDirectory[str] | None = None
    if args.output is None:
        owned = tempfile.TemporaryDirectory(prefix="ledgerguard-part2-stage8-")
        workspace = Path(owned.name)
    else:
        workspace = args.output.resolve()
        if repository == workspace or repository in workspace.parents:
            raise SystemExit("Part 2 Stage 8 output must be outside the repository")
        workspace.mkdir(parents=True, exist_ok=True)
    wheelhouses = [path.resolve() for path in args.wheelhouse]
    if any(not path.is_dir() for path in wheelhouses):
        raise SystemExit("Stage 8 wheelhouse does not exist")
    results = [
    ...
    if any(canonical(row) != first for row in deterministic[1:]):
        raise SystemExit("P2-S8-R018 clean-run deterministic payloads differ")
    evidence = {
        "schema_version": "1.0",
    ...
    destination = workspace / "part2-stage8-local-evidence.json"
