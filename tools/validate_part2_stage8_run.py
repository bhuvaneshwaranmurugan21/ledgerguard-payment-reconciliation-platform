#!/usr/bin/env python3
"""Validate one exact Part 2 Stage 8 promotion environment."""
...
from ledgerguard_part2_stage8_validation import validate_stage8
...
def main() -> None:
    ...
    if root == output.parent or root in output.parents:
        raise SystemExit("Stage 8 output must be outside the repository")
    if sys.version_info[:3] != (3, 11, 13):
        raise SystemExit("Stage 8 requires exact CPython 3.11.13")
    if pyspark.__version__ != "3.5.6" or version("py4j") != "0.10.9.7":
        raise SystemExit("Stage 8 Spark dependency versions differ")
    if (
        os.environ.get("PYSPARK_PYTHON") != sys.executable
        or os.environ.get("PYSPARK_DRIVER_PYTHON") != sys.executable
    ):
        raise SystemExit("Stage 8 Spark driver and worker interpreters differ")
    java = subprocess.check_output(
        ["java", "-version"], stderr=subprocess.STDOUT, text=True
    ).splitlines()[0]
    if '"17.' not in java:
        raise SystemExit(f"Stage 8 Java major differs: {java}")
    ...
    execute([sys.executable, "tools/build_part2_completion_ledgers.py", "--check"], root)
    ...
    execute(
        [
            ...
            "run",
            "--branch",
            "--source=ledgerguard_part2_stage8_validation",
            "-m",
            "pytest",
            str(root / "tests/test_part2_stage8_validation.py"),
        ],
        root,
    )
    ...
    authority = validate_stage8(root)
    ...
    result = {
        "actions": [
            ...
            "ruff-check",
            "mypy-strict",
            "generated-ledger-check",
            "full-pytest",
            "genuine-spark-regression",
            "stage8-branch-coverage",
            "stage8-semantic-mutations",
            "wheel-install-smoke",
        ],
