#!/usr/bin/env python3
"""Validate one clean Part 2 Stage 5 settlement environment."""
...
def main() -> None:
    ...
    if root == output.parent or root in output.parents:
        raise SystemExit("Stage 5 output must be outside the source repository")
    if sys.version_info[:3] != (3, 11, 13):
        raise SystemExit("Stage 5 requires exact CPython 3.11.13")
    ...
    if totals["percent_covered"] != 100.0:
        raise SystemExit("Stage 5 production coverage is not 100 percent")

    authority = validate_stage5(root)
