#!/usr/bin/env python3
"""Build immutable exact-head CI evidence for a draft Stage 2 pull request."""
...
def main() -> None:
    ...
    if not isinstance(pull_request, dict):
        raise SystemExit("Stage 2 CI evidence requires a pull_request event")
    if pull_request.get("draft") is not True:
        raise SystemExit("Stage 2 evidence must be built while the pull request is draft")
    ...
    envelope: dict[str, Any] = {
        ...
        "stage2_candidate_digest": authority["stage2_candidate_digest"],
    ...
    if errors:
        raise SystemExit(f"Stage 2 CI evidence is invalid: {errors[0].message}")
    ...
    evidence_path = destination / "part2-stage2-ci-evidence.json"
