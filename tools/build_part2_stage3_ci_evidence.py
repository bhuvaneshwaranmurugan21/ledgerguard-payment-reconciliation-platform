#!/usr/bin/env python3
"""Build immutable exact-head CI evidence for a draft Stage 3 pull request."""
...
def main() -> None:
    ...
    if not isinstance(pull_request, dict):
        raise SystemExit("Stage 3 CI evidence requires a pull_request event")
    if pull_request.get("draft") is not True:
        raise SystemExit("Stage 3 evidence must be built while the pull request is draft")
    if pull_request["base"]["sha"] != "55e78f76e76bff7562d43a3d001dbb74fd66d8fd":
        raise SystemExit("Stage 3 pull request base is not the frozen Stage 2 closure")
    ...
    envelope: dict[str, Any] = {
        ...
        "stage3_candidate_digest": authority["stage3_candidate_digest"],
        ...
        "stage2_closure_commit": authority["stage2_closure"]["commit"],
        "stage2_external_state": authority["stage2_closure"]["state"],
    ...
    schema = json.loads((ROOT / "spec/part2-stage3-ci-evidence-v1.schema.json").read_text())
    ...
    evidence_path = destination / "part2-stage3-ci-evidence.json"
