#!/usr/bin/env python3
"""Run Part 2 Stage 7 twice in exact independent Spark environments."""
...
def run_clean(
    ...
    environment["PYTHONHASHSEED"] = str(index * 101)
    ...
    for lock in ("part2-stage7-bootstrap.lock", "part2-stage7-py311.lock"):
        execute(
            [
                str(python),
                "-m",
    ...
    if len(wheels) != 1:
        raise SystemExit("Stage 7 expected exactly one LedgerGuard wheel")
    ...
    execute(
        [
            str(python),
            str(source / "tools/validate_part2_stage7_run.py"),
            "--root",
            str(source),
            "--output",
            str(result_path),
...
def main() -> None:
    ...
    if args.clean_runs < 2:
        raise SystemExit("P2-S7-R022 requires at least two clean runs")
    if sys.version_info[:3] != (3, 11, 13):
        raise SystemExit("Part 2 Stage 7 requires exact CPython 3.11.13")
    repository = Path(__file__).resolve().parents[1]
    owned: tempfile.TemporaryDirectory[str] | None = None
    if args.output is None:
        owned = tempfile.TemporaryDirectory(prefix="ledgerguard-part2-stage7-")
        workspace = Path(owned.name)
    else:
        workspace = args.output.resolve()
        if repository == workspace or repository in workspace.parents:
            raise SystemExit("Part 2 Stage 7 output must be outside the repository")
        workspace.mkdir(parents=True, exist_ok=True)
    wheelhouses = [path.resolve() for path in args.wheelhouse]
    if any(not path.is_dir() for path in wheelhouses):
        raise SystemExit("Stage 7 wheelhouse does not exist")
    results = [
    ...
    if any(canonical(row) != first for row in deterministic[1:]):
        raise SystemExit("P2-S7-R010 clean-run deterministic payloads differ")
    evidence = {
        "schema_version": "1.0",
    ...
    destination = workspace / "part2-stage7-local-evidence.json"
